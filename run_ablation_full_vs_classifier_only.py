...
def run_dataset(name: str, path: Path) -> list[dict[str, object]]:
    ...
    train_x, test_x, train_y, test_y, train_y_cat, _ = train_test_split(
    ...
    rows.append({
        "dataset": name,
        "model": "full_mlc_ae",
        "auc": f"{auc_from_probs(test_y, full_probs):.6f}",
        "n_cells": x.shape[0],
        "n_genes": x.shape[1],
        "epochs": EPOCHS,
        "batch_size": BATCH_SIZE,
    ...
    rows.append({
        "dataset": name,
        "model": "classifier_only_no_decoder",
        "auc": f"{auc_from_probs(test_y, class_probs):.6f}",
        "n_cells": x.shape[0],
        "n_genes": x.shape[1],
        "epochs": EPOCHS,
        "batch_size": BATCH_SIZE,
